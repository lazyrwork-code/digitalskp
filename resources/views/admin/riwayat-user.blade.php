...
@section('content')
<div class="content">
    <h3 class="fw-bold mb-4">Repository SKP - {{ $user->nama }}</h3>
    <div class="row g-4">
        <div class="col-md-3">
            <div class="bg-white rounded-4 p-3 shadow-sm h-100">
                <div class="d-grid gap-2">
                    @foreach(range(date('Y') + 2, date('Y') - 2) as $th)
                        <a href="{{ route('admin.riwayat-user', ['id_user' => $id_user, 'tahun' => $th]) }}"
            ...
            <div class="bg-white rounded-4 p-4 shadow-sm">
                ...
                <table class="table align-middle">
                    ...
                    <tbody>
                        ...
                        @empty
                        <tr>
                            <td colspan="4" class="text-center text-muted p-4">Belum ada riwayat pengajuan SKP.</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
                <div class="text-end mt-4">
                    <a class="btn btn-secondary px-4" href="{{ route('admin.riwayat') }}"><i class="bi bi-arrow-left"></i> Kembali</a>
                </div>
            </div>
        </div>
    </div>
