    ...
    <form action="{{ route('kepala.skp.update-status', $skp->id) }}" method="POST">
                                ...
                                <div class="koreksi-wrapper">
                                    ...
                                    <div class="koreksi-edit d-none">
                                        <textarea class="form-control koreksi-text" rows="2"
                                            name="koreksi[{{ $doc->id }}]">{{ $pesanKoreksi }}</textarea>
        ...
        <div class="bg-white p-4 rounded-4">
            ...
            <div class="d-flex gap-4 flex-wrap">
                <button type="button" class="btn btn-keputusan btn-perbaikan"
                    data-bs-toggle="modal" data-bs-target="#modalKembalikan">
                    <i class="bi bi-pencil-square me-2"></i> Kembalikan ke pegawai untuk perbaikan File
                </button>
                <button type="button" class="btn btn-keputusan btn-setujui"
                    data-bs-toggle="modal" data-bs-target="#modalSetujui">
                    <i class="bi bi-file-earmark-check me-2"></i> Tanda Tangan SKP Digital Selesai
                </button>
            </div>
        </div>
        <br />
        ...
        <div class="text-end mt-4">
            <a class="btn btn-secondary px-4" href="{{ route('kepala.dashboard') }}">
            ...
            <div class="modal-dialog modal-xl">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <div id="pdfViewer" style="position:relative;width:100%;height:600px;border:1px solid #ccc;overflow:auto;">
                            <div id="pdfPages"></div>
                        </div>
                    ...
                    <div class="modal-footer">
                        <button id="btnSimpanTTD" class="btn btn-success">Simpan TTD</button>
                    </div>
                </div>
            </div>
        </div>
...
<style>
#pdfViewer { height: 600px; overflow-y: auto; position: relative; }
#pdfPages { position: relative; }
.pdf-page { position: relative; margin: 0 auto 20px; }
#qrDrag { position: absolute; cursor: move; user-select: none; touch-action: none; z-index: 9999; }
</style>
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    let kepalaNip = @json(auth()->user()->nip);
    ...
    async function loadPDF(url) {
        ...
        for (let pageNum = 1; pageNum <= pdf.numPages; pageNum++) {
            ...
            pageWrapper.style.cssText = "position:relative;margin-bottom:20px;display:inline-block;box-shadow:0 0 10px rgba(0,0,0,0.2);";
    ...
    function generateQR() {
        ...
        qrCanvas.innerHTML = "";
        new QRCode(qrCanvas, {
            text: `ID: ${generatedVerifyId} | Tgl: ${timestamp} | TTD: ${kepalaNama} | NIP: ${kepalaNip}`,
            width: 80, height: 80,
    ...
    qrDrag.addEventListener("pointerdown", (e) => {
        ...
        if (qrDrag.parentElement !== pdfPages) pdfPages.appendChild(qrDrag);
    ...
    qrDrag.addEventListener("pointermove", (e) => {
        ...
        const pagesRect = pdfPages.getBoundingClientRect();
        let newX = Math.max(0, Math.min(e.clientX - pagesRect.left - offsetX, pdfPages.scrollWidth - qrDrag.clientWidth));
        let newY = Math.max(0, Math.min(e.clientY - pagesRect.top - offsetY, pdfPages.scrollHeight - qrDrag.clientHeight));
        qrDrag.style.left = newX + "px";
        qrDrag.style.top = newY + "px";
    });

    function stopDrag(e) {
        if (qrDrag.hasPointerCapture(e.pointerId)) qrDrag.releasePointerCapture(e.pointerId);
        isDragging = false;
    }
    ...
    document.getElementById("btnSimpanTTD").addEventListener("click", function () {
        ...
        document.querySelectorAll(".pdf-page").forEach(page => {
            ...
            if (qrTop >= pageTop && qrTop <= pageBottom) targetPage = page;
        });
